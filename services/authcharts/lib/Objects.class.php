...
class Objects {
		...
		$i = _hx_anonymous(array()); $c = _hx_anonymous(array()); $keys = Reflect::fields($a);
		{
			...
			while($_g < $keys->length) {
				...
				if(_hx_has_field($b, $key)) {
					...
					$i->{$key} = Objects::interpolateByName($key, $va)($va, Reflect::field($b, $key), null);
					unset($va);
				} else {
	...
	static function formatf($param, $params, $culture) {
		...
		case "R":{
			return array(new _hx_lambda(array(&$culture, &$format, &$param, &$params), "Objects_4"), 'execute');
		}break;
		default:{
			Objects_5($culture, $format, $param, $params);
		}break;
		}
	}
