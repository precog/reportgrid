...
class Dynamics {
	...
	static function interpolatef($a, $b, $equation) {
		...
		if(!((Std::is($a, _hx_qtype("Float")) || Std::is($a, _hx_qtype("Int"))) && (Std::is($b, _hx_qtype("Float")) || Std::is($b, _hx_qtype("Int")))) && !Type::enumEq($ta, $tb)) {
			throw new HException(new thx_error_Error("arguments a ({0}) and b ({0}) have different types", new _hx_array(array($a, $b)), null, _hx_anonymous(array("fileName" => "Dynamics.hx", "lineNumber" => 59, "className" => "Dynamics", "methodName" => "interpolatef"))));
		}
		$퍁 = ($ta);
		switch($퍁->index) {
		case 0:
		...
		$c = $퍁->params[0];
		{
			...
			default:{
				throw new HException(new thx_error_Error("cannot interpolate on instances of {0}", null, $name, _hx_anonymous(array("fileName" => "Dynamics.hx", "lineNumber" => 77, "className" => "Dynamics", "methodName" => "interpolatef"))));
			}break;
			}
		}break;
		default:{
			throw new HException(new thx_error_Error("cannot interpolate on functions/enums/unknown", null, null, _hx_anonymous(array("fileName" => "Dynamics.hx", "lineNumber" => 79, "className" => "Dynamics", "methodName" => "interpolatef"))));
		}break;
		}
	}
	...
	static function compare($a, $b) {
		if(!Types::sameType($a, $b)) {
			throw new HException(new thx_error_Error("cannot compare 2 different types", null, null, _hx_anonymous(array("fileName" => "Dynamics.hx", "lineNumber" => 131, "className" => "Dynamics", "methodName" => "compare"))));
		}
		if(null === $a && null === $b) {
			return 0;
		}
	...
	static function hclone($v, $cloneInstances) {
				...
				if($cloneInstances) {
					$o = Type::createEmptyInstance($c);
					{
						...
						while($_g < $_g1->length) {
							$field = $_g1[$_g];
							++$_g;
							$o->{$field} = Dynamics::hclone(Reflect::field($v, $field), null);
							unset($field);
						}
					}
					return $o;
